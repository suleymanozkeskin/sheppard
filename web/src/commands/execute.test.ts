...
const workspace: HerdrWorkspaceView = {
  ...
  label: "North",
  panes: [pane],
  tabs: [{ id: "current-tab", label: "Work", panes: [pane] }],
}
...
describe("command message writes", () => {
  ...
  test("a channel send does not join as a hidden side effect", async () => {
    ...
    const result = await sendCommandMessage(
      api,
      { kind: "channel", channel: "review", membership: "not-joined" },
      "Review this.",
    )
  ...
  test("uses the message endpoint for joined channels and direct groups", async () => {
    ...
    expect(
      (await sendCommandMessage(api, { kind: "channel", channel: "review", membership: "joined" }, "Ready.")).isOk(),
    ).toBe(true)
    expect(
      (
        await sendCommandMessage(api, { kind: "direct", channel: "dm-group", label: "worker, reviewer" }, "Ready.")
      ).isOk(),
    ).toBe(true)
  ...
  test("broadcast uses the workspace endpoint and reports its destination", async () => {
    ...
    const api = new HttpMsgrApi({
      ...
      fetchImpl: async (input) => {
        calls.push(String(input))
        return Response.json({ channel: "ws-north", messageId: 1, recipients: ["worker"] })
      },
    })
    const result = await sendCommandMessage(
      api,
  ...
  test("write failures keep rejection separate from uncertain completion", () => {
    ...
    expect(errors.map((error) => commandWriteFailure(error).kind)).toEqual([
      "outcome-unknown",
      "outcome-unknown",
      "outcome-unknown",
      "not-completed",
...
describe("current terminal targets", () => {
  test("focus refreshes topology and uses the current tab", async () => {
    ...
    const api = new HttpMsgrApi({
      ...
      fetchImpl: async (input) => {
        ...
        calls.push(path)
        return Response.json(path.endsWith("/workspaces") ? { workspaces: [workspace] } : { tabId: "current-tab" })
      },
    })
  ...
  test("terminal input is sent once to the current pane", async () => {
    ...
    const api = new HttpMsgrApi({
      ...
      fetchImpl: async (input) => {
        const path = String(input)
        calls.push(path)
        return Response.json(path.endsWith("/workspaces") ? { workspaces: [workspace] } : { delivered: true })
      },
    })
  ...
  test("inspection destinations preserve each audience", () => {
    expect(messageDestination({ kind: "agent", handle: "worker", routeState: "stale" })).toEqual({
      kind: "agent",
      handle: "worker",
    })
    expect(messageDestination({ kind: "channel", channel: "review", membership: "joined" })).toEqual({
      kind: "channel",
      channel: "review",
    })
    expect(messageDestination({ kind: "direct", channel: "dm-group", label: "Group" })).toEqual({
      kind: "conversation",
      channel: "dm-group",
    })
    expect(messageDestination({ kind: "broadcast", workspaceId: "w1", label: "North", recipients: [] })).toEqual({
      kind: "workspace",
      workspaceId: "w1",
    })
  })

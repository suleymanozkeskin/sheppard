...
describe("command drafts", () => {
  ...
  test("clearing editable text frees its slot", () => {
    const result = updateCommandDraft(new Map(), target, "Draft").andThen((drafts) =>
      updateCommandDraft(drafts, target, ""),
    )
    expect(result.isOk()).toBe(true)
  ...
  test("editing or clearing text cannot remove send uncertainty", () => {
    const result = updateCommandDraft(new Map(), target, "Draft").andThen((drafts) =>
      updateCommandDraft(setDraftDelivery(drafts, commandDraftKey(target), "uncertain"), target, ""),
    )
    expect(result.isOk()).toBe(true)
    if (result.isOk())
      expect(result.value.get(commandDraftKey(target))).toEqual({ target, body: "", delivery: "uncertain" })
  })

  test("limits new targets without removing current drafts", () => {
    const drafts: CommandDrafts = new Map(
      Array.from({ length: COMMAND_DRAFT_LIMIT }, (_, index) => [
        String(index),
        { target, body: "Draft", delivery: "editable" as const },
      ]),
    )

...
test("command menu opens with Cmd+K and keeps input focus while filtering", async ({ page }) => {
  ...
  await page.screenshot({ path: test.info().outputPath("sheppard-command-home.png") })
  ...
  await page.screenshot({ path: test.info().outputPath("sheppard-command-dark.png") })
...
test("keyboard-only category keys and focus stay visible in a narrow window", async ({ page }) => {
  ...
  await page.screenshot({ path: test.info().outputPath("sheppard-categories-mobile-search.png") })
  await page.keyboard.press("Tab")
  const categories = menu.getByRole("toolbar")
  for (const name of ["Agents", "Channels", "Workspaces", "Actions", "All"]) {
    await page.keyboard.press("ArrowRight")
  ...
  await page.screenshot({ path: test.info().outputPath("sheppard-categories-mobile-focused.png") })
  await page.setViewportSize({ width: 1280, height: 900 })
  await page.screenshot({ path: test.info().outputPath("sheppard-categories-desktop-focused.png") })
  await page.keyboard.press("ArrowDown")
  await expect(menu.getByRole("combobox")).toBeFocused()
  await page.screenshot({ path: test.info().outputPath("sheppard-categories-desktop-search.png") })
})
...
test("unconfirmed spawn keeps its setup and requires an explicit retry choice", async ({ page }) => {
  ...
  await page.screenshot({ path: test.info().outputPath("sheppard-command-spawn-mobile.png") })
  await submit.click()
...
test("keyboard-only back works from child searches, chrome, and empty results", async ({ page }) => {
  ...
  await page.screenshot({ path: test.info().outputPath("sheppard-command-navigation-desktop.png") })
  await page.setViewportSize({ width: 390, height: 650 })
  await expect(menu.getByRole("button", { name: "Back in command menu" })).toBeInViewport()
  await page.screenshot({ path: test.info().outputPath("sheppard-command-navigation-mobile.png") })
  await page.keyboard.press("ArrowLeft")
  await expect(search).toHaveValue("codex-reviewer")
  await page.keyboard.press("ArrowRight")
...
test("spawn arrows follow a narrow layout and skip an unavailable effort", async ({ page }) => {
  ...
  await page.screenshot({ path: test.info().outputPath("sheppard-form-arrows-mobile.png") })
  expect(writes).toEqual([])
})
  ...
  for (const width of [1280, 390] as const) {
    test(`spawn focus follows the complete field in ${colorScheme} at ${width}px`, async ({ page }) => {
      ...
      for (const name of SPAWN_PICKER_NAMES) {
        ...
        await expect(control).toHaveCSS("outline-width", "2px")
        await page.screenshot({ path: test.info().outputPath(`sheppard-focus-${colorScheme}-${width}-${name.toLowerCase()}.png`) })
        await page.keyboard.press("Escape")
...
test("agent workbench puts the session and composer within reach", async ({ page }) => {
  ...
  await page.screenshot({ path: test.info().outputPath("sheppard-agent-workbench-dark.png") })
  ...
  await expect(composer).toBeInViewport()
  await expect(composer).toBeFocused()
  await page.screenshot({ path: test.info().outputPath("sheppard-agent-workbench-mobile.png") })
...
test("agent session and chat share the full page height in a vertical split", async ({ page }) => {
  ...
  await page.screenshot({ path: test.info().outputPath("sheppard-agent-vertical-split.png") })
})
...
test("spawn setup keeps exact runtime values and survives Back", async ({ page }) => {
  ...
  await page.screenshot({ path: test.info().outputPath("sheppard-command-spawn.png") })
...
test("light and small-screen menus keep keyboard control", async ({ page }) => {
  ...
  await page.goto("/agents/codex-reviewer")
  await expect(page.getByRole("heading", { name: "Review complete" })).toBeVisible()
  await page.screenshot({ path: test.info().outputPath("sheppard-agent-workbench-light.png") })
  await page.keyboard.press("Meta+k")
  const menu = page.getByRole("dialog", { name: "Sheppard command menu" })
  const input = menu.getByRole("combobox")
  ...
  await expect(menu).toBeVisible()
  await page.screenshot({ path: test.info().outputPath("sheppard-command-light.png") })
  await page.setViewportSize({ width: 390, height: 844 })
  await input.fill("")
  await page.screenshot({ path: test.info().outputPath("sheppard-command-mobile.png") })
  expect(await page.evaluate(() => document.documentElement.scrollWidth <= innerWidth)).toBe(true)
